...
def _load_checkpoint(model: torch.nn.Module, path: str) -> None:
    ...
    print(
        f"loaded checkpoint={path} missing_keys={len(incompatible.missing_keys)} "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Decode one cached validation example with hierarchical PRA-AOG."
    )
    parser.add_argument("--bundle", required=True)
    parser.add_argument("--cache", required=True)
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--out-dir", default="runs/hier_pra_aog/inference")
    ...
    summary = {
        "sample_index": index,
        "ground_truth_class_id": int(batch["obj_label"][0].item()) if "obj_label" in batch else None,
        ...
        "subpart_bank_size": bundle.subpart_bank.count,
        "terminal_subpart_score": _jsonable(output.get("terminal_subpart_score")),
    }
