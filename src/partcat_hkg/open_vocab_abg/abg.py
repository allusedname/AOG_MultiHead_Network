...
def _class_margin(forest: OpenVocabParseForestV7) -> float:
    ...
    for hypothesis in forest.hypotheses:
        qid = int(hypothesis.object_query_id)
        best_by_query[qid] = max(best_by_query.get(qid, -1e9), float(hypothesis.score))
...
class OpenVocabEvidenceLedgerV7:
    ...
    def candidate_merge(self, results: list[OpenVocabRequeryResultV7]) -> list[OpenVocabTerminalV7]:
        ...
        for result in results:
            ...
            for terminal in result.terminals:
                # Replace weaker same-part evidence when the new ROI overlaps it.
                replaced = False
                for index, old in enumerate(merged):
                    ...
                    if _box_iou(old.packet.visible_box_xyxy, terminal.packet.visible_box_xyxy) >= 0.45:
                        if terminal.packet.visible_score > old.packet.visible_score:
    ...
    def commit(self, results: list[OpenVocabRequeryResultV7], merged: list[OpenVocabTerminalV7]) -> None:
        ...
        self.accepted_results.extend([r for r in results if r.accepted])
        self.rejected_results.extend([r for r in results if not r.accepted])
        self.queries.extend([r.query for r in results])


class OpenVocabularyABGEngineV7:
    """Open-object ABG loop with retrieval/generated grammars and unknown branch."""
    ...
    def _runtime_queries(self, object_texts: list[str], part_texts: list[str] | None = None) -> tuple[OpenVocabQueryBatchV7, list[OpenVocabObjectQueryV7]]:
        ...
        for index, text in enumerate(object_texts):
            qid = stable_query_id(f"object:{text}", namespace=1_000)
            query = OpenVocabTextQueryV7(qid, text, OpenVocabQueryKindV7.OBJECT)
            embedding = self.stage1.text_encoder.encode_query(query).detach().cpu()
            object_queries.append(query)
            object_specs.append(OpenVocabObjectQueryV7(qid, text, embedding))
    ...
    def _expected_box_from_relation(
        self,
        slot: DynamicSlotSpecV7,
        grammar: DynamicGrammarSpecV7,
        hypothesis: Any,
        terminal_by_id: dict[int, OpenVocabTerminalV7],
    ) -> tuple[float, float, float, float]:
        assignment = {a.slot_uid: a for a in hypothesis.slot_assignments if a.terminal_id is not None}
        slot_by_uid = {s.slot_uid: s for s in grammar.slots}
        for relation in sorted(grammar.relations, key=lambda r: r.reliability, reverse=True):
            ...
            ox = 0.5 * (observed[0] + observed[2])
            oy = 0.5 * (observed[1] + observed[3])
            ow = max(observed[2] - observed[0], 1e-4)
            oh = max(observed[3] - observed[1], 1e-4)
            source_mean = sibling_slot.geometry_mean
            target_mean = slot.geometry_mean
            dx = float(target_mean[0] - source_mean[0])
            dy = float(target_mean[1] - source_mean[1])
            width = max(0.02, float(target_mean[2]) * ow / max(float(source_mean[2]), 1e-4))
            height = max(0.02, float(target_mean[3]) * oh / max(float(source_mean[3]), 1e-4))
            cx = ox + dx
            cy = oy + dy
    ...
    def _gamma_queries(
        ...
        *,
        sample_id: int,
    ) -> list[OpenVocabGammaQueryV7]:
        grammar_by_query = {int(g.object_query.query_id): g for g in grammars}
        ...
        for hypothesis in forest.hypotheses:
            ...
            for assignment in hypothesis.slot_assignments:
                ...
                positive_context = [int(a.terminal_id) for a in hypothesis.slot_assignments if a.terminal_id is not None]
                candidates.append(OpenVocabGammaQueryV7(
                    ...
                    positive_context_terminal_ids=positive_context,
                ...
                self._next_query_id += 1
        # Deduplicate same object/slot and reserve some queries for distinct objects.
        ...
        for query in candidates:
            ...
            if key not in unique or query.priority > unique[key].priority:
                unique[key] = query
        rows = sorted(unique.values(), key=lambda q: q.priority, reverse=True)
    ...
    ) -> tuple[list[DynamicGrammarSpecV7], OpenVocabParseForestV7]:
        grammars = self.compiler.compile_many(object_specs, terminals=terminals, image_embedding=image_embedding)
        forest = self.parser.parse(grammars, terminals)
        return grammars, forest
    ...
    def run(
        ...
        *,
        ...
        terminals = self.stage1.terminals_from_output(output, image_hw=tuple(image.shape[-2:]), sample_ids=[sample_id])[0]
        ledger = OpenVocabEvidenceLedgerV7()
        ledger.add_global(terminals)
        ...
        for round_index in range(int(self.cfg.max_rounds)):
            ...
            accepted_visible = sum(1 for result in results if result.accepted for terminal in result.terminals if terminal.packet.accepted_visible)
            allow_commit = True
            if old_map is not None and new_map is not None and int(old_map.object_query_id) != int(new_map.object_query_id):
            ...
            if allow_commit:
                ...
                ledger.rejected_results.extend(results)
                forest_next = forest
            ...
            traces.append(OpenVocabABGRoundTraceV7(
                round_index=round_index,
                entropy_before=entropy_before,
                entropy_after=entropy_after,
                queries_emitted=len(queries),
                queries_accepted=sum(1 for result in results if result.accepted and allow_commit),
                class_margin_before=margin_before,
                class_margin_after=margin_after,
                map_object_before=map_before,
                map_object_after=map_after,
            ))
            ...
            if self.cfg.recompile_each_round and forest.map_parse is not None:
                grammar = next((g for g in grammars if int(g.object_query.query_id) == int(forest.map_parse.object_query_id)), None)
