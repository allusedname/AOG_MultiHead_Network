...
class ParserConfig:
    ...
    count_positive_scale: float = 1.00
    count_negative_scale: float = 1.00
    # v33: book-aligned group reasoning factors. These are explicit And-node
    # conjunction checks over the selected parse, complementary to pairwise
    # spatial/context edges and independent part-count likelihoods.
    reason_weight: float = 0.0
    reason_positive_scale: float = 1.0
    reason_negative_scale: float = 1.0
    ...
    edge_gate_floor: float = 0.20
    anchor_edge_weight: float = 1.00
    repeat_edge_weight: float = 1.00
    generic_edge_weight: float = 1.00
    structural_edge_weight: float = 1.00
    semantic_edge_weight: float = 1.00
    typed_edge_weight: float = 1.00
    edge_positive_scale: float = 1.00
    edge_negative_scale: float = 1.00
...
def _as_parser_config(cfg: Any) -> ParserConfig:
    ...
    return ParserConfig(
        ...
        count_positive_scale=float(_get(cfg, "strict_aog_count_positive_scale", 1.0)),
        count_negative_scale=float(_get(cfg, "strict_aog_count_negative_scale", 1.0)),
        reason_weight=float(_get(cfg, "strict_aog_reason_weight", 0.0)),
        reason_positive_scale=float(_get(cfg, "strict_aog_reason_positive_scale", 1.0)),
        reason_negative_scale=float(_get(cfg, "strict_aog_reason_negative_scale", 1.0)),
        ...
        edge_gate_floor=float(_get(cfg, "strict_aog_edge_gate_floor", 0.20)),
        anchor_edge_weight=float(_get(cfg, "strict_aog_anchor_edge_weight", 1.0)),
        repeat_edge_weight=float(_get(cfg, "strict_aog_repeat_edge_weight", 1.0)),
        generic_edge_weight=float(_get(cfg, "strict_aog_generic_edge_weight", 1.0)),
        structural_edge_weight=float(_get(cfg, "strict_aog_structural_edge_weight", 1.0)),
        semantic_edge_weight=float(_get(cfg, "strict_aog_semantic_edge_weight", 1.0)),
        typed_edge_weight=float(_get(cfg, "strict_aog_typed_edge_weight", 1.0)),
        edge_positive_scale=float(_get(cfg, "strict_aog_edge_positive_scale", 1.0)),
        edge_negative_scale=float(_get(cfg, "strict_aog_edge_negative_scale", 1.0)),
...
class StrictAOGParser(nn.Module):
    ...
    def __init__(self, grammar: StrictAOGGrammar, cfg: Any | None = None):
        ...
        self.register_buffer("edge_feature_mask", grammar.edge_feature_mask.float(), persistent=False)
        ...
        self.register_buffer("reason_rule_index", grammar.reason_rule_index.long(), persistent=False)
        self.register_buffer("reason_type", grammar.reason_type.long(), persistent=False)
        self.register_buffer("reason_min_count", grammar.reason_min_count.float(), persistent=False)
        self.register_buffer("reason_forbid_mask", grammar.reason_forbid_mask.float(), persistent=False)
        self.register_buffer("reason_support", grammar.reason_support.float(), persistent=False)
    ...
    @staticmethod
    def _diag_gaussian_ll(rel_vec: torch.Tensor, mu: torch.Tensor, var: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        var = var.to(rel_vec.device).clamp_min(1e-4)
        mu = mu.to(rel_vec.device)
        term = -0.5 * (((rel_vec - mu) ** 2) / var + var.log())
        if mask is None:
            return term.mean(-1)
        w = mask.to(rel_vec.device).float().clamp(0, 1)
        return (term * w).sum(-1) / w.sum().clamp_min(1.0)
    ...
    def _edge_ll(self, rel_vec: torch.Tensor, e: int) -> torch.Tensor:
        ...
        mask = self.edge_feature_mask[e].to(device) if self.edge_feature_mask.numel() else None
        ll_t = self._diag_gaussian_ll(rel_vec, mu_t, var_t, mask)
        mode = str(getattr(self.cfg, "edge_score_mode", "llr")).lower()
        if mode == "raw":
            val = ll_t
        else:
            mu_g, var_g, count_g = self._edge_background_params(e, device)
            ll_g = self._diag_gaussian_ll(rel_vec, mu_g, var_g, mask)
        ...
        pos_scale = float(getattr(self.cfg, "edge_positive_scale", 1.0))
        neg_scale = float(getattr(self.cfg, "edge_negative_scale", 1.0))
        val = torch.where(val > 0, val * pos_scale, val * neg_scale)
        ...
        floor = float(getattr(self.cfg, "edge_gate_floor", 0.20))
        floor = max(0.0, min(1.0, floor))
        etype = int(self.edge_type[e].detach().cpu().item()) if self.edge_type.numel() else 2
        type_weight = {
            0: float(getattr(self.cfg, "anchor_edge_weight", 1.0)),
            1: float(getattr(self.cfg, "repeat_edge_weight", 1.0)),
            2: float(getattr(self.cfg, "generic_edge_weight", 1.0)),
            3: float(getattr(self.cfg, "structural_edge_weight", 1.0)),
            4: float(getattr(self.cfg, "semantic_edge_weight", 1.0)),
        }.get(etype, float(getattr(self.cfg, "typed_edge_weight", 1.0)))
        return val * self.edge_support[e].to(device) * (floor + (1.0 - floor) * ig_gate) * type_weight
    ...
    def _edge_ll_batch(self, rel_pair: torch.Tensor, edge_idxs: torch.Tensor) -> torch.Tensor:
        ...
        feat_mask = self.edge_feature_mask.to(device)[edge_idxs].float().clamp(0, 1)
        feat_denom = feat_mask.sum(-1).clamp_min(1.0)
        x = rel_pair[:, None, :, :, :]
        term_t = -0.5 * (((x - mu_t[None, :, None, None, :]) ** 2) / var_t[None, :, None, None, :] + var_t.log()[None, :, None, None, :])
        ll_t = (term_t * feat_mask[None, :, None, None, :]).sum(-1) / feat_denom[None, :, None, None]
        mode = str(getattr(self.cfg, "edge_score_mode", "llr")).lower()
        if mode == "raw":
            val = ll_t
        else:
            ...
            term_g = -0.5 * (((x - mu_g[None, :, None, None, :]) ** 2) / var_g[None, :, None, None, :] + var_g.log()[None, :, None, None, :])
            ll_g = (term_g * feat_mask[None, :, None, None, :]).sum(-1) / feat_denom[None, :, None, None]
        ...
        floor = float(getattr(self.cfg, "edge_gate_floor", 0.20))
        floor = max(0.0, min(1.0, floor))
        et = self.edge_type.to(device)[edge_idxs]
        type_w = torch.full_like(ig, float(getattr(self.cfg, "typed_edge_weight", 1.0)))
        type_w = torch.where(et == 0, torch.full_like(type_w, float(getattr(self.cfg, "anchor_edge_weight", 1.0))), type_w)
        type_w = torch.where(et == 1, torch.full_like(type_w, float(getattr(self.cfg, "repeat_edge_weight", 1.0))), type_w)
        type_w = torch.where(et == 2, torch.full_like(type_w, float(getattr(self.cfg, "generic_edge_weight", 1.0))), type_w)
        type_w = torch.where(et == 3, torch.full_like(type_w, float(getattr(self.cfg, "structural_edge_weight", 1.0))), type_w)
        type_w = torch.where(et == 4, torch.full_like(type_w, float(getattr(self.cfg, "semantic_edge_weight", 1.0))), type_w)
        w = self.edge_support.to(device)[edge_idxs] * (floor + (1.0 - floor) * ig_gate) * type_w
    ...
    def _assigned_part_counts(self, batch: dict[str, torch.Tensor], c0: int, c1: int, assign: torch.Tensor | None) -> torch.Tensor:
        """Return selected parse part counts [B,Cc,A,K]."""
        device = self.class_prior.device
        ...
        if assign is None:
            return torch.zeros(B, Cc, A, kmax, device=device)
        valid = batch["terminal_valid"].to(device).float()
        part = batch["terminal_part"].to(device).long()
        safe_part = part.clamp(0, max(kmax - 1, 0))
        onehot = F.one_hot(safe_part, num_classes=kmax).float()
        terminal_use = assign.to(device).float().sum(dim=-2).clamp(0.0, 1.0)
        terminal_use = terminal_use * valid[:, None, None, :]
        return torch.einsum("bcan,bnk->bcak", terminal_use, onehot)

    def _reason_scores(self, batch: dict[str, torch.Tensor], c0: int, c1: int, assign: torch.Tensor | None) -> torch.Tensor:
        """Group-level reasoning score [B,Cc,A] from selected parse counts.

        A conjunct rule behaves like a soft logical AND: all required part-count
        clauses must be covered for positive evidence; missing any member drives
        the score negative.  Exclusion masks are soft penalties for selecting a
        part that contradicts a class/template explanation.
        """
        device = self.class_prior.device
        B = int(batch["terminal_valid"].shape[0])
        Cc = int(c1 - c0)
        A = int(self.grammar.num_templates)
        out = torch.zeros(B, Cc, A, device=device)
        if float(getattr(self.cfg, "reason_weight", 0.0)) == 0.0:
            return out
        if self.reason_rule_index.numel() == 0:
            return out
        obs_counts = self._assigned_part_counts(batch, c0, c1, assign)
        rows = self.reason_rule_index.to(device)
        keep = (rows[:, 0] >= int(c0)) & (rows[:, 0] < int(c1))
        if not bool(keep.any()):
            return out
        idxs = torch.nonzero(keep, as_tuple=False).flatten()
        min_counts = self.reason_min_count.to(device)
        forbid_masks = self.reason_forbid_mask.to(device)
        support = self.reason_support.to(device)
        for rr in idxs.tolist():
            c_abs = int(rows[rr, 0].item())
            c = c_abs - int(c0)
            a = int(rows[rr, 1].item())
            if c < 0 or c >= Cc or a < 0 or a >= A:
                continue
            req = min_counts[rr]
            forbid = forbid_masks[rr]
            score = torch.zeros(B, device=device)
            req_mask = req > 0
            if bool(req_mask.any()):
                cov = (obs_counts[:, c, a, req_mask] / req[req_mask].clamp_min(1.0)).clamp(0.0, 1.0)
                soft_and = cov.min(dim=-1).values
                score = score + 2.0 * soft_and - 1.0
            forbid_mask = forbid > 0
            if bool(forbid_mask.any()):
                violation = (obs_counts[:, c, a, forbid_mask].clamp(0.0, 1.0) * forbid[forbid_mask]).sum(-1)
                violation = violation / forbid[forbid_mask].sum().clamp_min(1.0)
                score = score - violation
            score = score * support[rr].clamp(0.0, 1.0)
            score = torch.where(
                score > 0,
                score * float(getattr(self.cfg, "reason_positive_scale", 1.0)),
                score * float(getattr(self.cfg, "reason_negative_scale", 1.0)),
            )
            out[:, c, a] += score
        return out
    ...
    def _part_count_scores(self, batch: dict[str, torch.Tensor], c0: int, c1: int, assign: torch.Tensor | None = None) -> torch.Tensor:
        ...
        pos_scale = float(getattr(self.cfg, "count_positive_scale", 1.0))
        neg_scale = float(getattr(self.cfg, "count_negative_scale", 1.0))
        val = torch.where(val > 0, val * pos_scale, val * neg_scale)
        return val
    ...
    def _score_chunk(self, batch: dict[str, torch.Tensor], c0: int, c1: int, *, enable_edges: bool) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, dict[str, torch.Tensor]]:
        ...
        reason = self._reason_scores(batch, c0, c1, assign=assign)
        ...
        template_scores = (
            ...
            + float(getattr(self.cfg, "reason_weight", 0.0)) * reason
            - validity_penalty
        )
        logits, best = self._aggregate_templates(template_scores, c0, c1)
        return logits, best, template_scores, node, edge, count, reason, stats
    ...
    def forward(self, batch: dict[str, torch.Tensor], *, enable_edges: bool = True, return_parse: bool = False) -> dict[str, Any]:
        ...
        reason_chunks: list[torch.Tensor] = []
        ...
        for c0 in range(0, C, chunk):
            c1 = min(C, c0 + chunk)
            lg, best, ts, ns, es, cs, rs, stats = self._score_chunk(batch, c0, c1, enable_edges=enable_edges)
        ...
        reason_scores = torch.cat(reason_chunks, dim=1)
        ...
        selected_reason = reason_scores[bidx, pred_for_diag, best_for_diag]
        ...
        out: dict[str, Any] = {
            ...
            "reason_logits": self._aggregate_edge_logits(reason_scores),
            ...
            "selected_reason_score": selected_reason,
            "reason_score_mean": selected_reason.mean(),
